...
bool BLEdeviceConnected = false; BLECharacteristic *characteristicTX; BLEScan *pBLEScan;
...
class CharacteristicCallbacks: public BLECharacteristicCallbacks {
     void onWrite(BLECharacteristic *characteristic) {
          ...
          if (rxValue.length() > 0) {
             ...
             Serial.print("BLE Received: "); Serial.println(charbuf);
               // Relay 1
...
void handleRelay1on()  { digitalWrite(relayGPIOs[0], 1); server.send(200, "text/plain", "OK"); }
void handleRelay1off() { digitalWrite(relayGPIOs[0], 0); server.send(200, "text/plain", "OK"); }
void handleRelay2on()  { digitalWrite(relayGPIOs[1], 1); server.send(200, "text/plain", "OK"); }
void handleRelay2off() { digitalWrite(relayGPIOs[1], 0); server.send(200, "text/plain", "OK"); }
...
bool readConfigFile() 
{
 ...
 if (!f) { return false; } else 
 {
  ...
  f.readBytes(buf.get(), size); f.close();
    
  DynamicJsonBuffer jsonBuffer;

  JsonObject& json = jsonBuffer.parseObject(buf.get());
  
  if (json.containsKey("mqttserver")) { strcpy(json_mqtt_server, json["mqttserver"]); }
  if (json.containsKey("mqttuser")) { strcpy(json_mqtt_user, json["mqttuser"]); }
  if (json.containsKey("mqttpass")) { strcpy(json_mqtt_pass, json["mqttpass"]); }
...
void led_format()
{
 for (uint8_t i; i<8; i++) { digitalWrite(led_wifi, !digitalRead(led_wifi)); delay(50); }
}

void mqttcallback(char* topic, byte* payload, unsigned int length)
{
  int i; char buffer[length]; for (i = 0; i<length; i++) { buffer[i] = payload[i]; }

  Serial.print("MQTT Topic:"); Serial.println(topic);
  Serial.print("MQTT Payload:"); Serial.println(buffer);
  
  if(!strcmp(topic, "relay1_switch") && buffer[0] == '1') { digitalWrite(relayGPIOs[0], 1); }
  if(!strcmp(topic, "relay1_switch") && buffer[0] == '0') { digitalWrite(relayGPIOs[0], 0); }
  
  if(!strcmp(topic, "relay2_switch") && buffer[0] == '1') { digitalWrite(relayGPIOs[1], 1); }
  if(!strcmp(topic, "relay2_switch") && buffer[0] == '0') { digitalWrite(relayGPIOs[1], 0); }
}
...
void manager_call()
{
  ...
  {
   server.stop();
   if (!SPIFFS.begin(true)) { SPIFFS.format(); Serial.println("Formatting Spiffs, please wait..."); }
   WiFi.disconnect(); WiFi.mode(WIFI_AP);
   digitalWrite(led_wifi, HIGH); delay(1000); SPIFFS.remove("/config.json"); led_format();
   ...
   if (writeConfigFile()) { Serial.println("Config Saved, Restarting..."); }
   ESP.restart();
  }
}
...
void start_services()
{
  server.begin(); Serial.println("Starting Webserver.");
...
void setup_wifi() 
{
  ...
  {
    digitalWrite(led_wifi, !digitalRead(led_wifi)); delay(100);
    if (!digitalRead(pin_config)) { manager_call(); } 
  }
...
void setup()
{
 ...
 for(int i=1; i<=NUM_RELAYS; i++){ pinMode(relayGPIOs[i-1], OUTPUT); }
 
 itoa(chipId, mqtt_clientid, DEC);
 
 pinMode(led_wifi, OUTPUT); digitalWrite(led_wifi, 1);  

 SPIFFS.begin(); delay(100); 
...
void loop()
{
  ...
  if (mqtt_enabled_c) { mqtt.loop(); }
  
  if (now - lastCheck > 1000) 
 {
  lastCheck = now; uptime++;
  if (!digitalRead(pin_config)) { manager_call(); } 
  
  if (WiFi.status() != WL_CONNECTED) { setup_wifi(); }
  if (!mqtt.connected() && (mqtt_enabled_c)) { reconnect(); }

  char charbuf[1];
  if(relayStates[0] != digitalRead(relayGPIOs[0])){
     relayStates[0] = digitalRead(relayGPIOs[0]);
     itoa(relayStates[0], charbuf, DEC);
     Serial.print("Relay1 State Changed to: "); Serial.println(charbuf);
     if (mqtt.connected()) { mqtt.publish("relay1_state", charbuf); }
   }

   if(relayStates[1] != digitalRead(relayGPIOs[1])){
     relayStates[1] = digitalRead(relayGPIOs[1]);
     itoa(relayStates[1], charbuf, DEC);
     Serial.print("Relay2 State Changed to: "); Serial.println(charbuf);
     if (mqtt.connected()) { mqtt.publish("relay2_state", charbuf); }
   }
